  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Profile creation skipped:', err);
      }

      // Step 3: Set as regular user role (not admin)
      try {
        ...
        if (roleError && !roleError.message.includes('duplicate')) {
          console.warn('⚠️ Role creation error:', roleError);
        } else {
          console.log('✅ User role created');
        }
      } catch (err) {
        console.warn('⚠️ Role creation skipped:', err);
      }
